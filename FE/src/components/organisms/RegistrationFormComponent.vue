    ...
    <div class="flex-1 p-8">
      ...
      <form @submit.prevent="onSubmit" class="space-y-4">
        ...
        <InputText id="email" label="Email" v-model="formData.email" placeholder="Email" type="email"
          :error="errors.email" @input="clearError('lastName')" />
        ...
        <InputText id="birthDate" label="Ngày sinh" v-model="formData.birthDate" placeholder="Ngày sinh" type="date"
          :error="errors.birthDate" @input="clearError('lastName')" />
...
import { DEFAULT_FORM_DATA, GENDER_OPTIONS } from '@/constants/formRegister';
...
const onSubmit = async () => {
  ...
  try {
    ...
    const response = await instanceAxios.post(ApiEndpoint.Register, formData.value);
    ...
    if (err instanceof z.ZodError) {
      ...
      errorMessage.value = axiosError.response?.data?.message || 'Đã có lỗi xảy ra, vui lòng thử lại!';
    }
  } finally {
