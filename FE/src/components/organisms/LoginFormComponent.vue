<template>
  <div
    ...
    <div class="flex-1 p-8 flex flex-col justify-center items-center h-auto max-h-[600px]">
      ...
      <form @submit.prevent="onLogin" class="space-y-4 w-full max-w-sm ">
        <InputText id="email" label="Email" v-model="formData.email" placeholder="Email" type="email"
          :error="errors.email" @input="clearError('email')" class="w-full" />
      ...
      <div class="mt-4 text-center text-base">
        ...
        <button @click="onForgotPassword" class="text-blue-700 hover:underline">
          vào đây
        </button>
      </div>
    ...
    <div class="flex-1 bg-[#704F38] text-white p-8 font-sans">
      <BoxText :text="'Đăng Ký'" @click="onRegister" />
      <div class="flex justify-center">
...
const onLogin = async () => {
  try {
    const formDataToSend: LoginFormData = {
    ...
    if (error instanceof AxiosError) {
      ...
      errorMessage.value = error?.response?.data?.message || "Đã xảy ra lỗi trong quá trình đăng nhập.";
    } else {
