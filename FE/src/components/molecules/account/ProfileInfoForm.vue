<template>
  <form @submit.prevent="$emit('submit')" class="space-y-8">
    ...
    <div v-if="isEditing" class="flex flex-col text-center sm:flex-row gap-4 pt-6 border-t border-gray-200">
      <button type="submit"
        class="flex-1 flex items-center justify-center bg-gradient-to-r from-gray-800 to-gray-900 hover:from-gray-900 hover:to-black text-white px-6 py-2.5 text-center text-lg font-medium shadow-md rounded">
        {{ $t('account.save') }}
      </button>
      <button type="button"
        class="flex-1 flex items-center justify-center border-2 border-gray-300 text-gray-700 hover:bg-gray-50 hover:border-gray-400 px-6 py-2.5 text-center text-lg font-medium rounded"
