        ...
        <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
          <div class="space-y-4">
            <div v-for="address in addresses" :key="address.id" 
                ...
                :class="{ 'border-neutral-800 bg-gray-50': address.id === modelValue }">
              <div class="flex">
                <input
                  type="radio"
                ...
                <div class="ml-3 flex-1">
                  <label :for="'modal-address-' + address.id" class="block text-lg font-medium text-gray-700">
                    ...
                    <span v-if="address.isDefaultShipping || address.isDefault" class="ml-2 inline-flex items-center px-2.5 py-0.5 rounded-full text-xl font-medium bg-green-100 text-green-800">
                      {{ $t('address.default') }}
                    </span>
                  </label>
                ...
                <button 
                  @click.stop="$emit('edit-address', address.id)" 
...
const selectedAddress = ref(props.modelValue || 0);

watch(() => props.modelValue, (newValue) => {
  selectedAddress.value = newValue || 0;
});

watch(() => props.isOpen, (isOpen) => {
  if (isOpen) {
    selectedAddress.value = props.modelValue || 0;
  }
});
