<template>
  <div class="product-gallery">
    <div
      class="main-image-container bg-white border border-gray-200 rounded-lg p-4 mb-4 flex items-center justify-center relative h-[400px]">
      <img v-if="currentImage" :src="getCachedImageUrl(currentImage.imageUrl)"
        :alt="currentImage.altText || 'Product image'" class="h-auto max-h-[350px] w-auto max-w-full object-contain" />
      <div v-else class="w-full h-[400px] flex items-center justify-center bg-gray-100 rounded">
        <span class="text-gray-400">No image available</span>
      </div>

      <button v-if="images.length > 1" @click="$emit('prev-image')"
      ...
      <button v-if="images.length > 1" @click="$emit('next-image')"
      ...
      <div class="absolute top-3 right-3 flex gap-2">
        <button @click="openZoomModal"
          class="p-2 bg-white bg-opacity-70 rounded-full hover:bg-opacity-100 transition shadow" title="Zoom">
    ...
    <div v-if="images.length > 1" class="thumbnails-container flex space-x-2 overflow-x-auto">
      <button v-for="(image, index) in images" :key="image.id" @click="$emit('select-image', index)"
        ...
        <img :src="getCachedImageUrl(image.imageUrl)" :alt="image.altText || `Thumbnail ${index + 1}`"
          class="max-h-full max-w-full object-contain" />
      </button>
    </div>

    <div v-if="showZoomModal" class="fixed inset-0 z-50 bg-black bg-opacity-75 flex items-center justify-center p-4"
      @click.self="closeZoomModal">
      <div class="relative max-w-4xl max-h-[90vh] bg-white rounded-lg p-4 w-full">
        <img v-if="modalImage" :src="getCachedImageUrl(modalImage.imageUrl)"
          :alt="modalImage.altText || 'Product image'" class="w-full h-auto max-h-[80vh] object-contain" />

        <button @click="closeZoomModal"
          class="absolute top-2 right-2 p-2 bg-white bg-opacity-70 rounded-full hover:bg-opacity-100 transition shadow">
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
        ...
        <button v-if="images.length > 1" @click.stop="prevImageInModal"
        ...
        <button v-if="images.length > 1" @click.stop="nextImageInModal"
...
const emit = defineEmits(['select-image', 'next-image', 'prev-image'])
